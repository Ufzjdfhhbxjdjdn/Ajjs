...
def split_sig_pieces(script):
    try:
        ...
        print(f'⚠️ split_sig_pieces error: {e}')
...
def parseTx(txn):
    ...
    for m in range(inp_nu):
        try:
            ...
            print(f'⚠️ parseTx input parsing error: {e}')
            return None
    rest = txn[cur:]
...
def get_rawtx_from_blockchain(txid):
    try:
        ...
        print(f'⚠️ Unable to fetch RawTx for {txid}: {e}')
        return None
...
def check_tx(address):
    ...
    try:
        ...
        if not rawdata:
            print('⚠️ Warning: No data received from API.')
            return [], []
        res = json.loads(rawdata)
        for tx in res:
            for j, vin in enumerate(tx.get("vin", [])):
                if vin.get("prevout", {}).get("scriptpubkey_address", "") == address:
                    txid.append(tx.get("txid", ""))
                    cdx.append(j)
        print(f'✅ Found {len(txid)} transactions involving the address.')
    except Exception as e:
        print(f'⚠️ Exception in check_tx: {e}')
        return [], []
    return txid, cdx
...
print(f'TXID list length: {len(txid)} | cdx list length: {len(cdx)}')
if not txid:
    print('⚠️ No transactions found. The script will continue but no data to analyze.')
...
for c in range(len(txid)):
    ...
    if rawtx is None:
        print(f'Skipping Tx: {txid[c]} (Failed to fetch rawtx)')
        continue
    try:
        m = parseTx(rawtx)
        e = getSignableTxn(m)
        ...
        print(f'Skipping Tx: {txid[c]} due to error: {e}')
        continue
